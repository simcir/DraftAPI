...
def build_recommendations(payload: DraftRecommendationRequest, loader: DataLoader) -> List[RecommendationItem]:
    ...
    enemy_focus_roles = [role for role in locked_enemy_roles if role not in locked_our_roles]
    ...
    color_index = _build_color_index(loader)
...
def _locked_roles(pick_ids: Set[int], role_index: Dict[int, Set[str]]) -> Set[str]:
    ...
    for champ_id in pick_ids:
        ...
        if len(roles) == 1:
            ...
            continue
        if len(roles) == 2:
            role_pair = tuple(roles)
            duo_role_counts[role_pair] = duo_role_counts.get(role_pair, 0) + 1
    for role_pair, count in duo_role_counts.items():
        if count >= 2:
            locked.update(role_pair)
    return locked
...
def _build_color_index(loader: DataLoader) -> Dict[int, Set[str]]:
...
def _build_role_index(loader: DataLoader) -> Dict[int, Set[str]]:
    index: Dict[int, Set[str]] = {}
    for role in ROLE_ORDER:
        try:
            profile = loader.role_profile(role)
        ...
        for champ in profile.get("champions", []):
            ...
            if champ_id not in index:
                index[champ_id] = set()
            index[champ_id].add(role)
    return index

def _build_enemy_role_index(champions: List[dict] | None) -> Dict[int, Set[str]]:
    if not champions:
        return {}
    index: Dict[int, Set[str]] = {}
    for champ in champions:
        ...
        if not roles:
            continue
        index[champ_id] = set(roles)
    return index

def _enemy_role_weights(
    enemy_picks: Set[int],
    role_index_enemy: Dict[int, Set[str]],
    locked_our_roles: Set[str],
    scoring_weights: Dict[str, float],
) -> Dict[str, float]:
    weights: Dict[str, float] = {}
    for champ_id in enemy_picks:
        roles = sorted(role_index_enemy.get(champ_id, []))
        if not roles:
            continue
        if len(roles) == 1:
            role = roles[0]
            ...
            dual_weight = _weight(scoring_weights, "roleDualMultiplier", 1.0)
            for role in roles:
                if role in locked_our_roles:
                    continue
                weights[role] = max(weights.get(role, 1.0), dual_weight)
    return weights
